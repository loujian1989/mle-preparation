...
| Role | Level | URL |
|---|---|---|
| Senior SWE/MLE, Pricing Algorithms — Auction Mechanisms | Senior | *(search greenhouse.io/moloco — job 7481629003; verify active before applying)* |

**Company:** Moloco — performance advertising AI company (~500 people, Series C, ~$200M raised, valued ~$1.5B). Builds ML-powered RTB (real-time bidding) infrastructure for mobile advertising networks and CTV. Seattle office confirmed active.

**TC note:** Pre-IPO startup — base ~$200K–$280K + equity options. Liquid TC ~$250K–$380K. **Does NOT reliably meet $500K+ liquid TC threshold.** However: strongest niche domain match of any company on this list (literally builds auction/pricing ML systems); high equity upside if IPO occurs; smaller team = higher ownership per person.

**Best fit for you:** Role owns pricing algorithms and auction mechanisms for Moloco's RTB platform — this is your exact domain. PhD game theory + Amazon SP autobidding + DoorDash Ads pacing = direct hire signal.

**JD Key Requirements (from Greenhouse, verified July 2026):**
- 5+ years building and deploying ML systems in production
- Deep expertise in auction pricing, bid optimization, or RTB systems
- ML for budget pacing, bid shading, win rate optimization, or supply/demand matching
- Python; experience with high-throughput low-latency serving (RTB is <100ms SLA)
- Preferred: PhD in CS, OR, Economics, or quantitative field; publications in auction theory or mechanism design

**Interview Process (startup style, from Greenhouse + LinkedIn, 2024–2026):**
1. Recruiter screen (30 min) — background, team fit, equity expectations
2. Technical screen (45–60 min) — coding + ML fundamentals (LeetCode medium)
3. Onsite (3–4 rounds):
   - **Coding** — LeetCode medium/hard; algorithms relevant to real-time systems
   - **ML system design** — design an RTB bidding system or auction pricing algorithm from scratch; latency, throughput, and feedback loop design all expected
   - **Domain deep-dive** — auction theory, bid shading in first-price environments, budget pacing math; expect derivations
   - **Hiring manager / values** — startup ownership mentality, high-ambiguity comfort
- Total timeline: 2–4 weeks

**Key insider notes:**
- This is the highest-niche-domain-fit company on the list — your PhD game theory + SP autobidding background is a very strong signal here
- RTB-specific prep: win rate prediction, bid shading (first-price equilibrium), pacing under RTB auction dynamics (different from guaranteed delivery pacing)
- Equity reality: pre-IPO options — negotiate strike price, cliff, and vesting schedule carefully; understand liquidation preference stack before accepting
- Startup ownership: bring stories about driving decisions under ambiguity, shipping without process, and owning both ML and product outcomes
- Moloco's ML differentiation is their bidding algorithm quality vs. larger platforms (Google/Meta) — they win on ML precision at smaller scale; frame your work in terms of efficiency gains, not just throughput
...
Your background is a genuine moat for most companies on this list. Here's how to deploy it surgically.
...
- RTB-specific: win rate prediction, bid shading in first-price auctions, real-time feedback loop design
- Derive bid shading equilibrium from first principles: in a first-price auction with uncertain opponent bids, the optimal shade factor is a function of the bid distribution; know the intuition even if not the closed form
- Startup framing: every answer should reflect ownership and ambiguity comfort — not just ML precision
- Equity negotiation: understand liquidation preference, option strike price, and cliff before signing
